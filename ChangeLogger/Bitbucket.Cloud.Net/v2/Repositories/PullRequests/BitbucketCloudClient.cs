	...
	public partial class BitbucketCloudClient
	{
		...
		public async Task<PullRequest> CreateRepositoryPullRequestAsync(string workspaceId, string repositorySlug, PullRequestCreationParameters pullRequestCreationParameters)
		{
			var data = new
			{
				...
				source = new { branch = new { name = pullRequestCreationParameters.Source.Name } },
				destination = new { branch = new { name = pullRequestCreationParameters.Destination.Name } },
				close_source_branch = pullRequestCreationParameters.CloseSourceBranch,
                reviewers = pullRequestCreationParameters.Reviewers.Select(r => new { username = r.Name }).ToArray()
			};
		    ...
		    try
		    {
                response = await GetPullRequestsUrl(workspaceId, repositorySlug)
		            .PostJsonAsync(data)
		            .ConfigureAwait(false);
		    }
		    catch (FlurlHttpException e)
		    {
		        Console.WriteLine($"ERROR: Please notify p.kanarek@insys.pl about this" +
		            $"Exception when creating pull request: {e}" +
		            $"Url: {e.Call.Request.RequestUri} " +
		            $"Headers {e.Call.Request.Headers}" +
		            $"Body: {JsonConvert.SerializeObject(data)} " +
		            $"Response: {e.Call.Response.ToString()}");
		        return null;
		    }
